...
import { clickAndWaitForApi } from './navigation';
...
test('library page renames current location and refreshes filtered items', async ({ page }) => {
  ...
  const moveButton = page.getByRole('button', { name: 'Переместить' });
  await expect(moveButton).toBeEnabled();
  await clickAndWaitForApi(page, moveButton, {
    url: `${BACKEND_URL}/api/library/rename-location`,
    method: 'PATCH'
  });
