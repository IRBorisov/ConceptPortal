...
import { withPdfExportProgress } from '../export-progress';
 ...
 * available. Shows the global PDF progress overlay while running.
 *
...
export function cstListToFile(data: Constituenta[], locale: AppLocale): Promise<Blob> {
  return withPdfExportProgress(() => renderCstListPdfInWorker(data.map(toConstituentaPdfRow), locale));
}
 ...
 * Shows the global PDF progress overlay while running.
 *
...
export function createSchemaFile(data: RSForm, locale: AppLocale): Promise<Blob> {
  return withPdfExportProgress(() => renderSchemaPdfInWorker(toSchemaPdfInput(data), locale));
}
