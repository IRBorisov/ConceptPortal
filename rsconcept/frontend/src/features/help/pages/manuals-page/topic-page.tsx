...
export function TopicPage({ topic }: TopicPageProps) {
  ...
  if (topic === HelpTopic.UI_RSMODEL_VALUE_DIALOG) return <HelpRSModelValueDialog />;
  if (topic === HelpTopic.UI_RSMODEL_EVALUATOR) return <HelpRSModelEvaluator />;
  if (topic === HelpTopic.UI_RSMODEL_STATUS) return <HelpRSModelStatus />;
  if (topic === HelpTopic.UI_RSMODEL_BINDING) return <HelpRSModelBinding />;
