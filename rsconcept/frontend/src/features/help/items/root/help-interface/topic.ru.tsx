...
export function HelpInterfaceRu() {
  const tx = useTx();
  return (
    <>
      ...
      <p>
        ...
        иконку <IconHelp className='inline-icon' />: при наведении — краткий просмотр темы, по нажатию открывается
        страница руководств.
      </p>
      <p>
        Значок <IconTour className='inline-icon' /> означает, что для этой области доступен интерактивный тур. При
        включённой контекстной справке наведение по-прежнему показывает просмотр темы; по нажатию открывается меню с{' '}
        <em>{tx('tx.help.quickGuide')}</em> и <em>{tx('tx.help.readManual')}</em>. Такая иконка остаётся видимой, даже
        если контекстная справка отключена.
      </p>
      <p>
      ...
      <p>
        Интерактивные туры проводят по текущему экрану шаг за шагом: подсветка указывает на элемент, а карточка
        объясняет его назначение.
      </p>
      <ul>
        <li>
          На некоторых экранах тур предлагается при первом посещении (например, в Песочнице). Его можно начать,
          пропустить или закрыть клавишей <kbd>Esc</kbd>.
        </li>
        <li>
          В других местах нажмите <IconTour className='inline-icon' /> и выберите <em>{tx('tx.help.quickGuide')}</em>,
          чтобы запустить тур по этой области — паспорт, список конституент, редактор понятия, граф терминов, библиотека
          и т.п.
        </li>
        <li>
          На обзорных шагах может быть ссылка <em>{tx('tx.general.details')}</em> для более подробного тура по вкладке;
          завершение или пропуск возвращает к обзору.
        </li>
        <li>
          Далее / Назад — переход по шагам, Пропустить — выход раньше, Завершить — на последнем шаге. Уход со страницы
          приостанавливает тур, чтобы продолжить при возврате.
        </li>
      </ul>
