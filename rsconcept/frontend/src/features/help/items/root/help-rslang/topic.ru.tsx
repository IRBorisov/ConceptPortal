...
export function HelpRSLangRu() {
  const tx = useTx();
  return (
    <>
      <h1>{tx('tx.rslang.short')}</h1>
      <p>
        Формальная запись (<i>экспликация</i>) концептуальных схем осуществляется с помощью языка родоструктурной
        экспликации (далее — <b>ЯРЭ</b>). В данном разделе поясняются основные понятия и формальные конструкции ЯРЭ. В
        своей основе ЯРЭ является формальной логикой первого порядка, все дополнительные обозначения основываются на
        предикате принадлежности <code>x∈y</code>.
      </p>
      <p>
      ...
      <p>
        Элементы ЯРЭ подразделяются на теоретико-множественные выражения и логические выражения. Упрощенно говоря,
      ...
      <p>
        ...
        <LinkTopic topic={HelpTopic.CC_CONSTITUENTA} text='терм-функций и предикат-функций' />.
      </p>
      ...
      <div className='dense'>
        <p>Материалы для ознакомления с основами ЯРЭ:</p>
        <p>
