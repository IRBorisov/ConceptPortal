...
export function HelpConceptPropagationRu() {
  const tx = useTx();
  return (
    <>
      ...
      <p>
        Изменения на уровне концептуальной схемы (добавление, удаление или изменение конституент) приводят к
        автоматическому созданию, удалению или обновлению наследованных конституент. Если удаляемые конституенты
        находятся в таблице отождествлений одной из операций, то такие отождествления{' '}
        <u>будут автоматически отменены</u>.
      </p>
      <p>
        После отмены отождествления на заново восстановленную конституенту не обновляются ссылки в собственных
        конституентах синтезированной схемы.
      </p>
      <p>
