...
export function HelpConceptRelationsRu() {
  const tx = useTx();
  return (
    <>
      ...
      <p>
        Если в формальном выражении используются только структурные формулы, то такое определение называется "простым"
        или "строго формальным". То есть для построения подобных определений достаточно формально применить конструкции
        языка, новые договоренности об использовании содержания предметной области не требуются.
      </p>
      <p>
