...
export function TabPromptVariables({ template }: TabPromptVariablesProps) {
  ...
  return (
    <ul>
      {variables.length === 0 ? <li>{tx('ui.aiPrompt.variables.none')}</li> : null}
      {variables.map(variable => {
        ...
        return (
          <li key={variable} className={isAvailable ? 'text-green-700 font-bold' : 'text-gray-500'}>
            {variable} — {type ? describePromptVariable(type) : tx('ui.aiPrompt.variables.unknown')}
            {isAvailable
              ? tx('ui.aiPrompt.variables.availableSuffix')
              : tx('ui.aiPrompt.variables.unavailableSuffix')}
          </li>
        );
      })}
    </ul>
