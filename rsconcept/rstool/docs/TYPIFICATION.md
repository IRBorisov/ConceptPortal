...
`R1`, `R2`, ... — шаблонные ступени в доменах параметров `F#` / `P#`. **Не конституенты:** в КС нет и не требуется записи с `alias` `R#`.

- Значение `R#` выводится по аргументам в месте вызова `F#[…]` / `P#[…]`.
- Все вхождения одного `R#` в одном определении должны дать одну ступень.
- Разные `R#` независимы.
- Радикалы можно писать только в доменах параметров (в `[…]`); в теле результата будет `radicalUsage`.
- При ревью схемы отсутствие `R1` среди конституент при наличии `R1` в формулах `F#` / `P#` — ожидаемое состояние, а не пробел ядра КС.
