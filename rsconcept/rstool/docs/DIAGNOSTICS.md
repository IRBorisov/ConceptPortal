# Диагностика

Читай, когда `analyzeExpression`, `applySchemaPatch`, `listDiagnostics` или `evaluate`
вернули диагностику. Коды ошибок ЯРЭ — `RSErrorCode` в `@rsconcept/domain`; коды схемы,
модели и таблицы отождествлений — `RSDiagnosticCode` в том же пакете (числовой формат совпадает;
`SubstitutionErrorType` — алиасы кодов отождествлений).

## Формы диагностики

**`CstDiagnostic`** (`@rsconcept/domain`, на `Constituenta` после загрузки схемы Portal) — только:

| Поле      | Смысл                                      |
| --------- | ------------------------------------------ |
| `kind`    | `schema` (пока только схема)               |
| `code`    | `RSDiagnosticCode`                         |
| `params?` | аргументы; омонимы/дубликаты — `[aliases]` |

**`Diagnostic`** (`@rsconcept/rstool`, `analyzeExpression`, `listDiagnostics`) — расширяет `CstDiagnostic` полями, удобными агенту:

| Поле         | Смысл                                                                  |
| ------------ | ---------------------------------------------------------------------- |
| `name`       | символьное имя (`typesNotEqual`, `schemaHomonym`, …) — ключ для агента |
| `severity`   | `error` \| `warning`                                                   |
| `alias?`     | обозначение конституенты                                               |
| `expression` | текст формулы или затронутого поля                                     |
| `from`, `to` | полуинтервал `[from, to)` в `expression`                               |
| `stack?`     | цепочка вызовов при ошибке внутри тела `F#`/`P#` (см. ниже)            |

Фильтры `listDiagnostics({ kind, constituentId, severity })`.

### Стек вызовов (`stack`)

При вычислении ошибки внутри вызванной терм-/предикат-функции (`E81xx` и любые
runtime-ошибки в теле `F#`/`P#`) диагностика несёт `stack` — кадры от места сбоя
наружу. Если ошибки нет в вызове (сбой в самом корневом выражении) — `stack`
отсутствует:

| Поле         | Смысл                                    |
| ------------ | ---------------------------------------- |
| `alias`      | конституента, в чьей формуле лежит span  |
| `from`, `to` | полуинтервал в формуле этой конституенты |

Порядок: **сначала место ошибки**, затем call site во внешнем вызывающем, …
Корневое выражение в стек не входит: его call site — в `from`/`to` самой
диагностики (подсветка в редакторе). Пример `F7[X1]` → `F8[…]` → `debool`:
`stack[0] = { alias: 'F8', … }`, `stack[1] = { alias: 'F7', … }`, а
корневой `F7[X1]` — только в `from`/`to`.

Форматирование для людей: `formatEvalCallStack` из `@rsconcept/domain` → `F7 → F8`.

## Где лежит диагностика

Три категории (`kind`), собранные в `listDiagnostics` после `applySchemaPatch`,
`setModelValues` и `recalculateModel`:

| `kind`       | Источник (как в Portal)                                                                              |
| ------------ | ---------------------------------------------------------------------------------------------------- |
| `expression` | разбор и семантика формулы (`definitionFormal`)                                                      |
| `schema`     | омонимы, дубликаты формул, пустой термин/конвенция у неопределяемых, расхождение типизации           |
| `model`      | статус интерпретации (`EvalStatus`): пустая база, ложная аксиома, неверные данные, ошибка вычисления |

## Схема (Portal)

| Код      | Имя                       | `params`             | Исправление                                                                                                                                         |
| -------- | ------------------------- | -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `0x8901` | `schemaHomonym`           | `[aliases]`          | Термин совпадает с другими конституентами (`aliases` через запятую). Переименуй термин.                                                             |
| `0x8902` | `schemaFormalDuplicate`   | `[aliases]`          | Нормализованная формула совпадает с `aliases`. Объедини или различай определения.                                                                   |
| `0x8903` | `schemaMissingConvention` | —                    | У `basic`/`constant`/`structure` нужна конвенция.                                                                                                   |
| `0x8904` | `schemaMissingTerm`       | —                    | У неопределяемого понятия нужен термин.                                                                                                             |
| `0x8905` | `schemaTypeMismatch`      | `[manual, computed]` | расхождение ручной и вычисленной типизации.                                                                                                         |
| `0x8906` | `schemaDependencyCycle`   | `[cycle]`            | Круг в графе определений (`F3 → P1 → F3`). Разорви цикл: убери взаимные ссылки. Тот же код — у таблицы отождествлений после применения подстановок. |

Поле `params` для омонимов/дубликатов — `[aliases]` (строка с alias через запятую).

## Таблица отождествлений

Коды в том же пространстве, что `RSDiagnosticCode` / `RSErrorCode`. Короткие имена `SubstitutionErrorType.*` — алиасы. Логический круг в определениях — **`schemaDependencyCycle` (`0x8906`)**, не отдельный код.

| Код      | Имя                               | `params`       | Исправление                                                        |
| -------- | --------------------------------- | -------------- | ------------------------------------------------------------------ |
| `0x8920` | `substitutionInvalidIDs`          | —              | Некорректные id схем/конституент в таблице.                        |
| `0x8921` | `substitutionIncorrectCst`        | `[from, to]`   | У одной из пары некорректное выражение.                            |
| `0x8922` | `substitutionInvalidClasses`      | `[from, to]`   | Классы конституент несовместимы для отождествления.                |
| `0x8923` | `substitutionInvalidBasic`        | `[from, to]`   | Нельзя заменять структурное понятие базисным множеством.           |
| `0x8924` | `substitutionInvalidConstant`     | `[from, to]`   | Константное множество только вместо другого константного.          |
| `0x8925` | `substitutionTypificationCycle`   | `[detail]`     | Цикл подстановок в типизациях.                                     |
| `0x8926` | `substitutionBaseNotSet`          | `[from, type]` | Типизация замещающей не задаёт множество.                          |
| `0x8927` | `substitutionUnequalTypification` | `[from, to]`   | Типизации операндов не совпадают с учётом таблицы.                 |
| `0x2928` | `substitutionUnequalExpressions`  | `[from, to]`   | Предупреждение: формальные определения различаются (не блокирует). |
| `0x8929` | `substitutionUnequalArgsCount`    | `[from, to]`   | Число аргументов параметризованных конституент не совпадает.       |
| `0x892A` | `substitutionUnequalArgs`         | `[from, to]`   | Типизации аргументов не совпадают.                                 |
| `0x892B` | `substitutionInvalidNominal`      | `[from, to]`   | Номиноид только вместо другого номиноида.                          |

## Модель (Portal)

Статусы соответствуют фильтру «проблемы модели» в UI (`isModelIssue`).

| Код      | Имя                | `params`   | Исправление                                                          |
| -------- | ------------------ | ---------- | -------------------------------------------------------------------- |
| `0x8910` | `modelEmpty`       | `[status]` | У базового/константного/структурного понятия нет данных в модели.    |
| `0x8911` | `modelAxiomFalse`  | `[status]` | Аксиома ложна на текущих данных — проверь формулу или интерпретацию. |
| `0x8912` | `modelInvalidData` | `[status]` | Данные не соответствуют типу (напр. элемент вне базы).               |
| `0x8913` | `modelEvalFail`    | `[status]` | Ошибка вычисления производного значения.                             |

Диагностики модели пересчитываются после `setModelValues` и `recalculateModel` (и при каждом `applySchemaPatch` для пустых баз).

## Класс и префикс кода

Класс выводится по битовым маскам кода (порядок проверки важен — см. `inferErrorClass`):

| Маска    | Класс      | Префикс `getRSErrorPrefix` |
| -------- | ---------- | -------------------------- |
| `0x0100` | EVALUATION | `E` + hex                  |
| `0x0200` | LEXER      | `L` + hex                  |
| `0x0400` | PARSER     | `P` + hex                  |
| `0x0800` | SEMANTIC   | `S` + hex                  |
| иначе    | UNKNOWN    | `U` + hex                  |

- Диапазоны: `0x84xx` — parser (синтаксис, скобки), `0x88xx` — semantic (типизация, область
  видимости, структура, схема, отсутствие значения), `0x81xx` — evaluation (вычисление КМ),
  `0x89xx` — schema / model / substitution diagnostics (`RSDiagnosticCode`),
- `0x2xxx` — предупреждения (не блокируют, `isCritical` возвращает `false`; в т.ч. `0x2928` unequalExpressions).

## Parser

| Код      | Имя                        | `params`                  | Исправление                                                                      |
| -------- | -------------------------- | ------------------------- | -------------------------------------------------------------------------------- |
| `0x8400` | `unknownSyntax`            | —                         | Неопределённая синтаксическая ошибка: проверь операторы, скобки, идентификаторы. |
| `0x8401` | `forbiddenCharacter`       | `[character]`             | Символ не входит в ЯРЭ (напр. `^` для степени). Удали или замени его.            |
| `0x8409` | `bracketMismatch`          | `[expected, actual]`      | Закрой ожидаемой скобкой `expected` вместо `actual`.                             |
| `0x840A` | `doubleParenthesis`        | —                         | Убери лишнюю внешнюю пару `(( … ))`.                                             |
| `0x840B` | `missingOpenBracket`       | `[bracket]`               | Непарная закрывающая `bracket`: убери её или добавь открывающую.                 |
| `0x840C` | `missingCloseBracket`      | `[bracket]`               | Непарная открывающая `bracket`: убери её или добавь закрывающую.                 |
| `0x8415` | `expectedLocal`            | —                         | Ожидалось имя локальной переменной.                                              |
| `0x8416` | `expectedType`             | `[expectedClass, actual]` | Несоответствие класса выражения ожидаемому.                                      |
| `0x8417` | `invalidFilterSyntax`      | —                         | Синтаксис `Fi`: `Fi1[D1](S1)`, `Fi1,2[D1,D2](S1)` или `Fi1,2[D3](S1)`.           |
| `0x8418` | `expectedFunctionBody`     | —                         | Нужно тело функции после `[параметры]`.                                          |
| `0x8419` | `expectedExpressionBody`   | —                         | Нужно теоретико-множественное тело функции (ТФ).                                 |
| `0x841A` | `expectedLogicBody`        | —                         | Нужно логическое тело функции.                                                   |
| `0x841B` | `expectedQuantifierBody`   | —                         | Нужно логическое тело после `∀(∃)…∈домен`.                                       |
| `0x841C` | `expectedDeclarativeBody`  | —                         | Допиши декларативное `D{…∈… \| …}`.                                              |
| `0x841D` | `expectedImperativeBody`   | —                         | Допиши императивное `I{… \| …}`.                                                 |
| `0x841E` | `expectedRecursiveBody`    | —                         | Допиши рекурсивное `R{…:=… \| … \| …}`.                                          |
| `0x841F` | `expectedQuantifierDomain` | —                         | После переменной квантора нужны `∈` и домен.                                     |
| `0x8420` | `expectedRightOperand`     | —                         | Нужен правый операнд после оператора.                                            |
| `0x8421` | `expectedUnaryOperand`     | —                         | Нужно выражение после `¬`.                                                       |
| `0x8422` | `globalFuncParenCall`      | `[name]`                  | Вызов `F`/`P` только через `[…]`, не `(…)`.                                      |
| `0x8423` | `expectedArgument`         | —                         | Нужен аргумент в списке `[…]` у F/P.                                             |

## Предупреждения

| Код      | Имя            | `params` | Исправление                        |
| -------- | -------------- | -------- | ---------------------------------- |
| `0x2802` | `localNotUsed` | `[name]` | Переменная `name` не используется. |

## Локальные переменные и область видимости

| Код      | Имя                          | `params`         | Исправление                                                                                   |
| -------- | ---------------------------- | ---------------- | --------------------------------------------------------------------------------------------- |
| `0x8801` | `localUndeclared`            | `[name]`         | Свяжи `name` квантором или объяви параметром.                                                 |
| `0x8802` | `localShadowing`             | `[name]`         | Переименуй внутреннюю переменную `name`.                                                      |
| `0x8803` | `localDoubleDeclare`         | `[name]`         | Идентификатор `name` уже объявлен в выражении — переименуй одну из переменных.                |
| `0x8815` | `localOutOfScope`            | `[name]`         | Перенеси использование `name` внутрь её области видимости.                                    |
| `0x882E` | `localOutOfScopeParentheses` | `[name]`         | Переменная `name` вне области квантора: возьми тело квантора в скобки (напр. `∀a∈S (… & …)`). |
| `0x882F` | `localUndeclaredInSubexpr`   | `[name, domain]` | В подвыражении нет квантора для `name`; ожидается `∀(∃) name∈domain`.                         |

## Типизация и структура

| Код      | Имя                                | `params`                           | Исправление                                                                                                                                                                                                                      |
| -------- | ---------------------------------- | ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `0x880A` | `typesNotEqual`                    | `[a, b, operator]`                 | Операнды `operator` должны иметь равный тип (`a` ≠ `b`).                                                                                                                                                                         |
| `0x8825` | `typesNotCompatible`               | `[a, b, operator]`                 | Типы `a` и `b` несовместимы для `operator`.                                                                                                                                                                                      |
| `0x8804` | `globalNotTyped`                   | `[name]`                           | Поставщик `name` не типизирован: добавь и проанализируй его раньше. Если `name` в круге определений, анализ схемы/выражения может заменить код на `schemaDependencyCycle` (также для `globalFuncWithoutArgs` у голых `F#`/`P#`). |
| `0x8805` | `invalidDecart`                    | `[arg]`                            | Аргумент `arg` оператора `×` неподходящей ступени.                                                                                                                                                                               |
| `0x8806` | `invalidBoolean`                   | `[arg]`                            | Аргумент `arg` оператора `ℬ()` неподходящей ступени.                                                                                                                                                                             |
| `0x8810` | `invalidReduce`                    | `[arg]`                            | `red(arg)` требует ступень `ℬℬ`.                                                                                                                                                                                                 |
| `0x8807` | `invalidTypeOperation`             | `[operator, type]`                 | Аргумент `type` оператора `operator` (`∪`, `∈`, `D{}`, `:∈` и т.п.) должен быть множеством.                                                                                                                                      |
| `0x8808` | `invalidCard`                      | `[arg]`                            | `card(arg)` только над множеством.                                                                                                                                                                                               |
| `0x8809` | `invalidDebool`                    | `[arg]`                            | `debool(arg)` требует ступень-множество с гарантией синглетона.                                                                                                                                                                  |
| `0x880B` | `globalFuncWithoutArgs`            | `[name]`                           | Терм-/предикат-функция `name` вызвана без аргументов: нужен `F#[…]` / `P#[…]`.                                                                                                                                                   |
| `0x8830` | `projectionSetArgumentNotSet`      | `[operator, actual]`               | `Pr*` применяется к множеству; аргумент не типизирован как `ℬ(H)`.                                                                                                                                                               |
| `0x8831` | `projectionSetArgumentNotTupleSet` | `[operator, actual]`               | `Pr*` требует множество кортежей `ℬ(H1×…×Hn)`; аргумент — множество не-кортежей, напр. `ℬ(X1)`.                                                                                                                                  |
| `0x8832` | `projectionSetIndexOutOfRange`     | `[operator, index, arity, actual]` | Индекс проекции `index` превышает размерность `arity` кортежа аргумента `actual`.                                                                                                                                                |
| `0x8833` | `projectionTupleArgumentNotTuple`  | `[operator, actual]`               | `pr*` применяется к кортежу `H1×…×Hn`; получен другой тип.                                                                                                                                                                       |
| `0x8834` | `projectionTupleIndexOutOfRange`   | `[operator, index, arity, actual]` | Индекс `index` превышает размерность `arity` кортежа `actual`.                                                                                                                                                                   |
| `0x8813` | `invalidEnumeration`               | `[a, b]`                           | Элементы перечисления разной типизации (`a` ≠ `b`).                                                                                                                                                                              |
| `0x8814` | `invalidCortegeDeclare`            | —                                  | Число переменных кортежа ≠ размерности `×` (напр. `∀(a,b)∈X1`).                                                                                                                                                                  |
| `0x8816` | `invalidElementPredicate`          | `[a, operator, c]`                 | `a operator c` (`ξ∈S`) с несовместимыми ступенями: `a` — тип элемента, `c` — тип множества.                                                                                                                                      |
| `0x8817` | `invalidEmptySetUsage`             | —                                  | Бессмысленное использование `∅` как нейтрального элемента операций (напр. `X1∪∅`). Сравнение `S=∅` / `S≠∅` допустимо.                                                                                                                                 |
| `0x8818` | `invalidArgsArity`                 | `[expected, actual]`               | Неверное число аргументов функции (`expected` ≠ `actual`).                                                                                                                                                                       |
| `0x8819` | `invalidArgumentType`              | `[expected, actual]`               | Тип аргумента не соответствует объявлению: ожидалось `expected`, получено `actual`.                                                                                                                                              |
| `0x881C` | `globalStructure`                  | —                                  | Некорректный домен родовой структуры (`cstType` STRUCTURED): нужна ступень-структура вроде `ℬ(X1×D2)`, а не, напр., `X1∩X1`.                                                                                                     |
| `0x8821` | `radicalUsage`                     | `[name]`                           | Радикал `name` (`R#`) допустим только в домене параметра `[…]` у `F#` / `P#`, не в теле формулы. `R#` не объявляется конституентой — см. `SYNTAX.md`.                                                                            |
| `0x8824` | `arithmeticNotSupported`           | `[type, operator]`                 | `type` не поддерживает арифметику `operator`: нужна ступень `Z` или `C#`.                                                                                                                                                        |
| `0x8826` | `orderingNotSupported`             | `[type, operator]`                 | `type` не поддерживает порядок `operator`: нужна ступень `Z` или `C#`.                                                                                                                                                           |
| `0x8827` | `expectedLogic`                    | `[type]`                           | Нужна логическая формула, получен тип `type`.                                                                                                                                                                                    |
| `0x8828` | `expectedSetexpr`                  | `[type]`                           | Нужно теоретико-множественное выражение, получен тип `type` (напр. `Logic`).                                                                                                                                                     |
| `0x8829` | `invalidArgumentCortegeDeclare`    | —                                  | Связное объявление кортежа запрещено в аргументах функции (`[(a,b)∈S1]`): используй проекции или раздельные переменные.                                                                                                          |
| `0x882A` | `invalidQuantifierDomain`          | `[operator, type]`                 | Область квантора `operator` (`∀`/`∃`) должна быть множеством, а не `type`.                                                                                                                                                       |

## Фильтр `Fi`

| Код      | Имя                           | `params`                             | Исправление                                                                                                                                                       |
| -------- | ----------------------------- | ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `0x8822` | `invalidFilterArgumentType`   | `[operator, actual, expected]`       | Аргумент `Fi` имеет тип `actual`, ожидался `expected` (обычно `ℬ(…)`).                                                                                            |
| `0x8823` | `invalidFilterArity`          | `[indexCount, paramCount, operator]` | Число индексов `indexCount` ≠ числу параметров `paramCount` у `operator`.                                                                                         |
| `0x882B` | `invalidFilterParameterType`  | `[param, expected, operator]`        | Тип параметра `param` не совпадает с компонентом аргумента `expected` для `operator`.                                                                             |
| `0x882C` | `invalidFilterIndex`          | `[operator, actual, index, arity]`   | Индекс проекции `index` превышает арность кортежа `arity` аргумента `actual`.                                                                                     |
| `0x882D` | `invalidFilterBooleanEchelon` | `[operator, actual]`                 | Ступень параметра `Fi` на один ℬ ниже (`actual`; `bool(actual)` совпал бы с ожидаемым): элемент — в `{…}`, иначе множество нужной ступени или проверка семантики. |

## Значение и схема

| Код      | Имя                    | `params`           | Исправление                                                                                                                                                                                                                           |
| -------- | ---------------------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `0x8840` | `globalNoValue`        | `[name]`           | Идентификатор `name` невычислим в модели.                                                                                                                                                                                             |
| `0x8841` | `invalidPropertyUsage` | —                  | `Property` - множество использовано как вычислимое значение (типично: квантор по `ℬ(S1)` / `ℬ(X1)`). Перепиши через вычислимые термы (`F#` достижимости, `card`, `Pr`) или оставь как декларативную формулировку и укажи это в ревью. |
| `0x8861` | `cstEmptyDerived`      | —                  | У выводимой конституенты пустая формула.                                                                                                                                                                                              |
| `0x8862` | `definitionNotAllowed` | `[cstType, alias]` | У `basic`/`constant` формула должна быть пустой; `cstType` — роль конституенты, `alias` — её обозначение.                                                                                                                             |

## Вычисление КМ

| Код      | Имя                       | `params`           | Исправление                                                                            |
| -------- | ------------------------- | ------------------ | -------------------------------------------------------------------------------------- |
| `0x8100` | `calcUnknownError`        | —                  | Непредвиденная ошибка вычисления.                                                      |
| `0x8101` | `setOverflow`             | `[limit]`          | Превышен предел числа элементов `limit`: упрости выражение или данные.                 |
| `0x8102` | `booleanBaseLimit`        | `[limit]`          | Превышен предел базы степенного множества `limit`.                                     |
| `0x8103` | `calcGlobalMissing`       | `[name]`           | Нет значения для `name`: задай через `setModelValues`.                                 |
| `0x8104` | `iterationsLimit`         | `[limit]`          | Превышен предел итераций `limit`.                                                      |
| `0x8105` | `calcInvalidDebool`       | —                  | `debool` получил не синглетон. Смотри `stack`, если в исходном выражении нет `debool`. |
| `0x8106` | `iterateInfinity`         | —                  | Итерация по бесконечному множеству.                                                    |
| `0x8107` | `calculationNotSupported` | —                  | Объявление функции не подразумевает вычисления.                                        |
| `0x8108` | `calcInvalidData`         | `[a, b, operator]` | Несовместимые значения `a` и `b` для `operator` в runtime.                             |

## Цикл исправления

Пошаговый цикл — в `../skills/rstool-helper/GUIDE.md`, раздел «Цикл диагностик». Здесь — справочник кодов.
