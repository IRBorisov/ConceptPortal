...
- `R#` — **радикал**: шаблонная ступень в **доменах параметров** терм-функции (`F#`) или предикат-функции (`P#`). Это не конституента: в схеме нет записи с `alias` `R1`, `R2`, … и добавлять её через правку схемы не нужно. Ступень `R#` выводится в **месте вызова** `F#[…]` / `P#[…]` по типизации аргументов.

Правило: `X#`, `C#`, `S#` задаются конвенцией и интерпретацией; `D#`, `F#`, `P#`, `A#`, `T#` выводятся по формуле. Поставщики должны идти раньше потребителей. Радикалы `R#` к конституентам не относятся.
