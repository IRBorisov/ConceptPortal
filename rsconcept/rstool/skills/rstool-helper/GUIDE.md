...
3. Проверь независимость базовых понятий — они не выводятся друг из друга.
   Если отношение или множество выражается через уже введенные `X#`, `C#`, `S#`, оформляй его как `D#` / `F#` / `P#`.
4. Введи `F#` и `D#` для раскрытия проекций и функциональных связей в базовых понятиях.
...
| Не пиши пользователю         | Пиши                                                          |
| :--------------------------- | :------------------------------------------------------------ |
| «базис», «базисный слой»     | **базовые понятия** (`X#`, `C#`, `S#`) или перечисли типы     |
| «базисное» про `S1`          | **родовая структура** `S1`                                    |
| «базовое« множество про `X1` | **базисное множество** `X1`                                   |
| «ядро» без уточнения         | **ядро КС** (если речь именно о ядре) или **базовые понятия** |
...
- [ ] Базовые понятия и `S#` независимы; вычислимые отношения вынесены в `D#` / `F#` / `P#`, а не объявлены новыми структурами.
