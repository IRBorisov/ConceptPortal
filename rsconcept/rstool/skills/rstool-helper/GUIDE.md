...
4. Структурный проход по [чеклисту ревью](#ревью-концептуальной-схемы) и [CONCEPTUAL-SCHEMA.md](../../docs/CONCEPTUAL-SCHEMA.md):
   ...
   - (не)пустота через `S ≠ ∅` / `S = ∅`, а не `card(S) > 0`;
...
5. Severity в отчёте — по **смыслу** диагностики, а не только по полю `severity` (инструмент помечает `error` и метаданные):
   ...
   - **важно** — нарушение правил проектирования (лишнее `S#`, график вместо `F#`, смешение экспликаций, перекрёстные ссылки `C#`, термин сильнее формулы, `card(S) > 0` вместо `S ≠ ∅`);
...
- [ ] Нет неофициальных ярлыков («базис», «базисы», «базисный слой», «базовые понятия») — термины КС из [DOMAIN.md](../../docs/DOMAIN.md), раздел **«Термины: не путать»**. Для `X#` говори **базисное множество**, не «базис».
...
- [ ] (Не)пустота множества записана как `S ≠ ∅` / `S = ∅` (или `A ∩ B ≠ ∅`), а не через `card(S) > 0` / `card(S) = 0` (см. [CONCEPTUAL-SCHEMA.md](../../docs/CONCEPTUAL-SCHEMA.md), «Пустота и мощность»).
