...
rstool re-exports `RSToolErrorCode.formalDefinitionNotAllowed` (`0x8862`) for base/constant violations.
...
- `0x81xx` — evaluation (runtime; usually not rstool session builds)
...
| rstool adapter | `rsconcept/rstool/src/mappers/frontend-domain-adapter.ts` |
...
After `npm run example:build-sample`, export contains `state.items[]` each with `alias`, `cstType`, `definitionFormal`, and nested `analysis` — use as a golden pattern for agent-generated JSON.
